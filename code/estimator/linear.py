...
def _compute_fraction_of_zero(variables):
  ...
  Returns:
    The fraction of zeros (sparsity) in the linear model.py.
  """
...
def _sdca_model_fn(features, labels, mode, head, feature_columns, optimizer):
  ...
  # tf.get_variables() for variable creation. So we modify the model.py name to
...
class _SDCAUpdateWeightsHook(session_run_hook.SessionRunHook):
  """SessionRunHook to update and shrink SDCA model.py weights."""
...
class LinearClassifierV2(estimator.EstimatorV2):
  """Linear classifier model.py.

  Train a linear model.py to classify instances into one of multiple possible
  ...
  * for each `column` in `feature_columns`:
  ...
  def __init__(self,
    ...
    Args:
      feature_columns: An iterable containing all the feature columns used by
        the model.py. All items in the set should be instances of classes derived
        from `FeatureColumn`.
      model_dir: Directory to save model.py parameters, graph and etc. This can
        also be used to load checkpoints from the directory into a estimator
        to continue training a previously saved model.py.
      ...
      optimizer: An instance of `tf.Optimizer` or
        `tf.estimator.experimental.LinearSDCA` used to train the model.py. Can
  ...
  def __init__(self,
    ...
    Args:
      ...
      model_dir: Directory to save model.py parameters, graph and etc. This can
        also be used to load checkpoints from the directory into a estimator
        to continue training a previously saved model.py.
      optimizer: An instance of `tf.Optimizer` used to train the model.py. Can also
...
class LinearRegressorV2(estimator.EstimatorV2):
  ...
  Train a linear regression model.py to predict label value given observation of
  feature values.
  ...
  def __init__(self,
    ...
    Args:
      ...
      model_dir: Directory to save model.py parameters, graph and etc. This can
        ...
        continue training a previously saved model.py.
      ...
      optimizer: An instance of `tf.Optimizer` or
        `tf.estimator.experimental.LinearSDCA` used to train the model.py. Can also
        be a string (one of 'Adagrad', 'Adam', 'Ftrl', 'RMSProp', 'SGD'), or
        callable. Defaults to FTRL optimizer.
      config: `RunConfig` object to configure the runtime settings.
      warm_start_from: A string filepath to a checkpoint to warm-start from, or
